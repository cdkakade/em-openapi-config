...
public class OpenApiConfig {
	...
	private Info info() {
		return new Info().title(configProperties.getTitle())
	...
	/*
	 * @Bean public OpenApiCustomiser globalErrorResponseOpenApiCustomiser(
	 *
	 * @Autowired OpenApiMessageProperties messageProperties) { return openAPI -> {
	 *
	 * if (Boolean.TRUE.equals(configProperties.getErrorResponse())) {
	 * openAPI.getComponents().getSchemas().putAll(ModelConverters.getInstance().read(
	 * ApiErrorRes.class)); Schema<ApiErrorRes> errorResponseSchema = new Schema<>();
	 ...
	 * pathItem.readOperations().forEach(operation -> { ApiResponses apiResponses =
	 * operation.getResponses();
	 * apiResponses.addApiResponse(String.valueOf(BAD_REQUEST.value()), createApiResponse(
	 * BAD_REQUEST.getReasonPhrase(), messageProperties.getBadRequest(),
	 * errorResponseSchema));
	 * apiResponses.addApiResponse(String.valueOf(UNAUTHORIZED.value()),
	 * createApiResponse( UNAUTHORIZED.getReasonPhrase(),
	 * messageProperties.getUnAuthorized(), errorResponseSchema));
	 * apiResponses.addApiResponse(String.valueOf(FORBIDDEN.value()), createApiResponse(
	 * FORBIDDEN.getReasonPhrase(), messageProperties.getForbidden(),
	 * errorResponseSchema));
